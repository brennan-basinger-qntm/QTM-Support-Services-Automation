...
# .\Find-MailboxAccess.ps1 -UserEmail "koko.breadmore@quantinuum.com"
...
function Resolve-IdentityStringToSet {
    ...
    try {
        ...
        # Keep the raw value only if it does not resolve
    }
...
foreach ($mbx in $allMailboxes) {
    ...
    try {
        ...
        foreach ($entry in $faEntries) {
            ...
            if (Test-IdentityIntersection -Left $trusteeSet -Right $targetIdentitySet) {
                Add-ResultRow -Results $results `
                    -MailboxDisplayName $mailboxName `
                    -MailboxAddress $mailboxAddress `
                    -MailboxType $mailboxType `
                    -AccessType 'FullAccess' `
                    -GrantedVia 'Direct' `
                    -MatchedTrustee $trusteeRaw
    ...
    try {
        ...
        foreach ($delegate in $sobDelegates) {
            ...
            if ([string]::IsNullOrWhiteSpace($delegateRaw)) {
                continue
            }
